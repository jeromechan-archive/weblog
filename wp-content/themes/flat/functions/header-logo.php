...
$option = get_flat_option(); //オプションの値を取得
if($logo){
...
function dp_logo_img_tag($resized = false, $alt = '', $aditional_class = '', $max_width = 0){
	$class = 'flat-logo ';
	$class .= $resized ? 'flat-logo-resized' : 'flat-logo-original';
...
function dp_logo_basedir(){
	$dir = wp_upload_dir();
	return $dir['basedir'].DIRECTORY_SEPARATOR.'flat';
}
...
function _dp_upload_logo(){
	...
	foreach($existing_files as $file){
		if(preg_match("/logo(-resized)?\.(png|gif|jpe?g)$/i", $file)){
			...
			if(!@unlink($dir.DIRECTORY_SEPARATOR.$file)){
				$dp_upload_error = array(
					'error' => true,
					'message' => sprintf(__('Cannot delete existing file <code>%s</code>', 'flat'), $file)
				);
				return $dp_upload_error;
			}
		}
	...
	//メッセージを更新する
	$dp_upload_error['message'] = __('Logo file was successfully uploaded.', 'flat');
	return  $dp_upload_error;
}
...
function _dp_delete_image(){
	if(isset($_REQUEST['page'], $_REQUEST['_wpnonce']) && !isset($_REQUEST['settings-updated']) && $_REQUEST['page'] == 'theme_options'){
		...
		$options = get_flat_option();
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_header_bg_image')){
				$file = basename($options['header_bg_image']);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['header_bg_image'] = '';
...
function _dp_delete_message_error(){
	echo '<div id="message" class="error"><p>'.sprintf(__('Failed to delete image. Please check permisson of %s. All files must be writable.', 'flat'), dp_logo_basedir()).'</p></div>';
}
...
function _dp_delete_message_sucess(){
	echo '<div id="message" class="updated fade"><p>'.__('Images are successfully deleted.', 'flat').'</p></div>';
}
...
function _dp_resize_logo(){
	...
	$info = dp_logo_info();
	if(!$info){
		$dp_resize_message['message'] = __('Logo file not exists.', 'flat');
	...
	if($dest_path && !@unlink($dest_path)){
		$dp_resize_message['message'] = __('Cannot delete existing resized logo.', 'flat');
		return $dp_resize_message;
	}
	//名前を決めて保存
	...
	if($result){
		...
		$dp_resize_message['message'] = __('Logo image is successfully resized.', 'flat');
	}else{
		$dp_resize_message['message'] = sprintf(__('Failed to save resized logo. Please check permission of <code>%s</code>', 'flat'), dp_logo_basedir());
	}
	return $dp_resize_message;
}
...
function _dp_get_upload_err_msg($error_code){
	...
	case UPLOAD_ERR_INI_SIZE:
		return __('Uploaded file size is larger than limit set in php.ini.', 'flat');
		break;
	case UPLOAD_ERR_FORM_SIZE:
		return __('Uploaded file size is larger than post file size.', 'flat');
		break;
	case UPLOAD_ERR_PARTIAL:
		return  __('The file has been uploaded only partially. Connection error may have occured', 'flat');
		break;
	case UPLOAD_ERR_NO_FILE:
		return  __('Uploaded file size is too large.', 'flat');
		break;
	case UPLOAD_ERR_NO_TMP_DIR:
		return  __('No temporary directory exists. Please check PHP Setting is valid.', 'flat');
		break;
	case UPLOAD_ERR_CANT_WRITE:
		return  __('Failed to write to disk. OS file setting or something is wrong.', 'flat');
		break;
	case UPLOAD_ERR_EXTENSION:
		return  __('PHP extension module stops uploading. Check PHP setting.', 'flat');
		break;
	default:
		return  __('Upload failed. Sorry, but reasons cannot be detected.', 'flat');
		break;
	}
}